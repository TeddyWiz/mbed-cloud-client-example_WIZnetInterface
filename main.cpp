...
void main_application(void)
{
 ...
 (MBED_CONF_EVENTS_SHARED_DISPATCH_FROM_APPLICATION == 1)
    ...
    printf("netmask : %s \r\n", Wiz_netmask);
    printf("dns     : %s \r\n", Wiz_dnsaddr);
    eth.init(WIZNET_DEFAULT_TESTMAC, Wiz_ip, Wiz_netmask, Wiz_gateway);
    eth.setDnsServerIP(Wiz_dnsaddr);
    ...
    if((result = UDP_socket.recvfrom(&Wiz_dest, buff, 256)) > 0)
    {
        printf("rec[%d] : %s \n",result, buff);
    }
    while(1)
    {
        ...
        result = UDP_socket.recvfrom(&Wiz_dest, buff, 256);
        if( result > 0)
        {
            printf("rec[%d] : %s \n",result, buff);
            send_result = UDP_socket.sendto(Wiz_dest,buff,result);
        }
    }
    //--------------------------------------
